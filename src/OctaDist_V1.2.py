...
def quit_msg():
    ...
    yes_btn = Button(frame1, text="Yes", bg="blue", fg="yellow", command=lambda: quit, width=10)
    yes_btn.pack(padx=10, pady=10, side=LEFT)
    no_btn = Button(frame1, text="No", bg="blue", fg="yellow", command=lambda: qw.destroy, width=10)
...
def check_gaussian_type(f):
    """Check is the input file is Gaussian output file
    """
...
def get_coord_from_gaussian(f):
    ...
    for line in numline[start + 5: end]:
        ...
        #print("%s%s" % (word1, line[30:-1]), end="", file=save_temp)
...
def get_coord(f):
    ...
    if check_gaussian_type(f) == 1:
        print("         File type: Gaussian output")
        get_coord_from_gaussian(f)

    elif check_xyz_type(f) == 1:
        print("         File type: XYZ file")
        get_coord_from_xyz(f)

    else:
        print("         File type: TXT file")
        get_coord_from_txt(f)
    ...
    textBox_coord.insert(INSERT, texts)
...
def calc_delta(x):
    ...
    for i in range(6):
        diff_dist = (distance_list[i] - distance_avg(x)) / distance_avg(x)
...
def calc_sigma(v):
    ...
    # Print new list of angle after three transs angle deleted
...
def calc_theta(v):
    ...
    print("         Total number of deleted angles  :", q)
...
# coordinate label
lbl_1 = Label(master, text="Molecule Specifications")
lbl_1.grid(sticky=W, pady="5", row=3, columnspan=4)

# text box for showing cartesian coordinates
textBox_coord = Text(master, height="14", width="65", wrap="word")
...
#################################
# Widgets for graphical displays
#################################
